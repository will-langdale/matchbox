...
class Dedupes(SHA1Mixin, CMFBase):
    ...
    left: Mapped[bytes] = mapped_column(ForeignKey("cmf__source_data.sha1"))
...
class DDupeProbabilities(CMFBase):
    ...
    ddupe: Mapped[bytes] = mapped_column(
        ForeignKey("cmf__ddupes.sha1"), primary_key=True
    )
...
class DDupeContains(UUIDMixin, CMFBase):
    ...
    child: Mapped[bytes] = mapped_column(ForeignKey("cmf__source_data.sha1"))
...
class DDupeValidation(UUIDMixin, CMFBase):
    ...
    ddupe: Mapped[bytes] = mapped_column(ForeignKey("cmf__ddupes.sha1"))
    user: Mapped[str]
    valid: Mapped[bool]
