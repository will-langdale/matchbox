...
def generate_cluster_source(
    ...
    Args:
        ...
        pk_start_id: Starting ID for primary keys (if None, will use cluster_start_id)
    ...
    if pk_start_id is None:
        pk_start_id = cluster_start_id + range_left
...
def generate_result_tables(
    ...
    prob_min: float = 0.6,
    prob_max: float = 1,
) -> tuple[list[int], pa.Table, pa.Table, pa.Table, pa.Table, int]:
