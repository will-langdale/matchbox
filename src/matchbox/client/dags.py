...
DAGNode = Union["ModelStep", SourceReader]
...
class StepInput(BaseModel):
    ...
    prev_node: DAGNode
    select: dict[SourceReader, list[str]]
    cleaners: dict[str, dict[str, Any]] = {}
    threshold: float | None = None

    @property
    def name(self):
        """Name within the DAG of node which output this input."""
        if isinstance(self.prev_node, SourceReader):
            return str(self.prev_node.address)
        else:
...
class ModelStep(BaseModel, ABC):
    ...
    def init_sources(self) -> "ModelStep":
        ...
        for step_input in self.inputs:
            if isinstance(step_input.prev_node, SourceReader):
...
class DAG:
    ...
    def add_sources(self, *sources: SourceReader):
        """Add sources to DAG.
    ...
    def run(self):
        ...
        for step_name in self.sequence:
            ...
            if isinstance(node, SourceReader):
