...
class Step(BaseModel, ABC):
    ...
    name: ResolutionName
    sources: set[SourceResolutionName] = Field(default_factory=set)
...
class StepInput(BaseModel):
    ...
    def validate_all_input(self) -> "StepInput":
        ...
        if isinstance(self.prev_node, IndexStep):
            if (
                len(self.select) > 1
                or list(self.select.keys())[0] != self.prev_node.config
            ):
                raise ValueError(
                    f"Can only select from source {self.prev_node.config.location.uri}"
                )
        else:
            for source in self.select:
                if source.name not in self.prev_node.sources:
                    raise ValueError(
...
class IndexStep(Step):
    ...
    def run(self) -> None:
        """Run indexing step."""
        _handler.index(source=self.config, batch_size=self.batch_size)
