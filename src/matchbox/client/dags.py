...
@runtime_checkable
class Step(Protocol):
    """Protocol defining what a step needs to support."""
...
class ModelStep(BaseModel):
    ...
    def __init__(self, **data):
        """Initialise model step."""
        super().__init__(**data)
        self._init_sources()

    def _init_sources(self):
        """Initialise sources from inputs."""
        self._sources = set()
        for step_input in self.inputs:
            if isinstance(step_input.prev_node, Source):
                self._sources.add(str(step_input.prev_node.address))
            else:
                self._sources.update(step_input.prev_node.sources)
