...
!!! tip "DAG steps and resolutions"
    Once it is run, a step in a DAG corresponds to a resolution in the Matchbox database. Learn more by reading about the tutorial on [exploring resolutions](/client/explore_resolutions) or dive deeper by consulting the extended guide to [Matchbox concepts](/server/concepts). 
