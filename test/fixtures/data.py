...
def query_clean_crn(db_engine: Engine) -> DataFrame:
    ...
    # Clean
    col_prefix = f"{os.getenv('MB__POSTGRES__SCHEMA')}_crn_"
...
def query_clean_duns(db_engine: Engine) -> DataFrame:
    ...
    # Clean
    col_prefix = f"{os.getenv('MB__POSTGRES__SCHEMA')}_duns_"
    cleaner_name = cleaner(
        function=company_name, arguments={"column": f"{col_prefix}company_name"}
    )
...
def query_clean_cdms(db_engine: Engine) -> DataFrame:
    ...
    select_cdms = selector(
        table=f"{os.getenv('MB__POSTGRES__SCHEMA')}.cdms",
        fields=["crn", "cdms"],
        engine=db_engine,
    )
...
def query_clean_duns_deduped(db_engine: Engine) -> DataFrame:
    ...
    duns = query(
        ...
        model=f"naive_{os.getenv('MB__POSTGRES__SCHEMA')}.duns",
        return_type="pandas",
        engine=db_engine,
    )
...
def query_clean_cdms_deduped(db_engine: Engine) -> DataFrame:
    ...
    cdms = query(
        ...
        model=f"naive_{os.getenv('MB__POSTGRES__SCHEMA')}.cdms",
        return_type="pandas",
        engine=db_engine,
    )
