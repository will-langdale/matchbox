...
def test_generate_entity_probabilities():
    ...
    results1 = ResultsEntity(source_pks=EntityReference({"dataset1": frozenset({"1"})}))
    results2 = ResultsEntity(source_pks=EntityReference({"dataset1": frozenset({"2"})}))
...
def test_generate_entity_probabilities_single():
    ...
    source = SourceEntity(
        ...
        source_pks=EntityReference({"dataset1": frozenset({"1"})}),
    )
    result = ResultsEntity(source_pks=EntityReference({"dataset1": frozenset({"1"})}))
    ...
    result2 = ResultsEntity(source_pks=EntityReference({"dataset1": frozenset({"1"})}))
    table = generate_entity_probabilities(
...
def test_probability_ranges(
    ...
    results = ResultsEntity(source_pks=EntityReference({"dataset1": frozenset({"1"})}))
...
def test_probability_recovers_source_entities():
    ...
    source2 = SourceEntity(
        ...
        source_pks=EntityReference(
            {
                "dataset1": frozenset({"3", "4"}),
                "dataset2": frozenset({"B"}),
            }
        ),
    )
    ...
    results = {
        ...
        ResultsEntity(source_pks=EntityReference({"dataset1": frozenset({"3"})})),
        ResultsEntity(
            source_pks=EntityReference(
