...
def test_match_ok(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    matchbox_api.get(f"/resolution/{source_testkit.source_config.name}").mock(
        return_value=Response(
            200, json=source_testkit.source_config.model_dump(mode="json")
        )
    )
    matchbox_api.get(f"/resolution/{target1_testkit.source_config.name}").mock(
        return_value=Response(
            200, json=target1_testkit.source_config.model_dump(mode="json")
        )
    )
    matchbox_api.get(f"/resolution/{target2_testkit.source_config.name}").mock(
        return_value=Response(
...
def test_match_404_resolution(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    matchbox_api.get(f"/resolution/{target_testkit.source_config.name}").mock(
        return_value=Response(
...
def test_match_404_source(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    matchbox_api.get("/resolution/source").mock(
        return_value=Response(
            404,
            json=NotFoundError(
