...
def test_select_default_engine(
    ...
    # Mock API
    matchbox_api.get(f"/sources/{testkit.config.name}").mock(
        return_value=Response(200, json=testkit.config.model_dump())
    )
...
def test_select_mixed_style(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    source1 = linked.sources["crn"].config
    source2 = linked.sources["cdms"].config
        ...
        return_value=Response(
            ...
            content=table_to_buffer(
                pa.Table.from_pylist(
                    [
                        {"source_identifier": "0", "id": 1},
                        {"source_identifier": "1", "id": 2},
                    ],
...
def test_query_multiple_sources(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    # Mock API
    matchbox_api.get(f"/sources/{testkit1.config.name}").mock(
        return_value=Response(200, json=testkit1.config.model_dump())
    )

    matchbox_api.get(f"/sources/{testkit2.config.name}").mock(
        return_value=Response(200, json=testkit2.config.model_dump())
    )
        ...
        side_effect=[
            ...
            Response(
                ...
                content=table_to_buffer(
                    pa.Table.from_pylist(
                        [
                            {"source_identifier": "2", "id": 1},
                            {"source_identifier": "3", "id": 2},
                        ],
                        schema=SCHEMA_MB_IDS,
                    )
                ).read(),
...
def test_query_404_source_get(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    # Mock API
    address = SourceAddress.compose(full_name="foo", credentials=sqlite_warehouse)
    ...
    matchbox_api.get("/query").mock(
        return_value=Response(
            ...
            content=table_to_buffer(
                pa.Table.from_pylist(
                    [{"source_identifier": "0", "id": 1}], schema=SCHEMA_MB_IDS
                )
            ).read(),
        )
    )
...
def test_query_with_batches(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    # Mock API responses
    matchbox_api.get(f"/sources/{source_testkit.config.name}").mock(
        return_value=Response(200, json=source_testkit.config.model_dump())
    )
    matchbox_api.get("/query").mock(
        return_value=Response(
            200,
...
def test_index_with_batch_size(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    with patch.object(
        SourceConfig, "hash_data", wraps=source_testkit.config.hash_data
    ) as spy_hash_data:
...
def test_match_ok(matchbox_api: MockRouter, sqlite_warehouse: Engine):
    ...
    # Use match function
    source = select({"test.config": ["a", "b"]}, credentials=sqlite_warehouse)
